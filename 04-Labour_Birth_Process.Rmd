...
# add data
# If it is a population health indicator, we typically want, all NS residents
# (i.e. ProvNo=12)
# If we're thinking about comparing facilities, we want all deliveries (births) in a (usually) Nova Scotia facility (i.e. a hospital number between 3 and 91)
# If it's looking a regional hospital, it is usually restricted to This Regional
# vs. All Other Regionals vs. The Whole Province (3<DLHosp<91)
# All deliveries/births that are (a) not terminations [except where mother's life is in imminent danger], and (b) that fit one or more of the following criteria - (i) liveborn, (ii) >= 500g, (iii) 20 weeks G.A., (iv) co-multiple of an infant meeting the other criteria, and, of course (c) that occur in Nova Scotia, or elsewhere but to a resident of Nova Scotia [though, currently, we only look at Moncton Hospital]
...
```{r plot410, fig.cap= paste('Type of delivery among candidates for vaginal birth after Caesarean (VBAC) who had any labour by year, Nova Scotia,', paste0(params$year1,"-",params$year2))}
