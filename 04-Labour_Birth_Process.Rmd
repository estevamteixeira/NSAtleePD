...
dta1 <- dta[, .(BrthYear,
  ...
  .[, `:=` (rate = count/total)] %>%
  .[,.(BrthYear, rate, total, ind, parity)] %>%
  unique()
...
plot <- ggplot(data = dta1[!ind %in% c("No")],
               aes(x = BrthYear,
                   ...
                   fill = parity,
                   shape = parity,
                   text =paste(paste0(parity," - ", BrthYear),
...
dta1 <- dta[, .(BrthYear,
  ...
  .[, `:=` (rate = count/total)] %>%
  .[,.(BrthYear, rate, total, ind_reason)] %>%
  unique()
...
plot <- ggplot(data = dta1,
               aes(x = BrthYear,
                   ...
                   fill = ind_reason,
                   shape = ind_reason,
                   text =paste(paste0(ind_reason," - ", BrthYear),
                               "<br>Total deliveries:", scales::comma(total, accuracy = 1),
...
plot <- ggplot(data = dta1[!aug %in% "No"],
               aes(x = BrthYear,
                   ...
                   fill = aug,
                   #shape = aug,
                   text =paste(paste0(aug," - ", BrthYear),
...
dta1 <- dta[, .(BrthYear,
  ...
  .[, `:=` (rate = count/total)] %>%
  .[,.(BrthYear, rate, total, anes, deliv)] %>%
  unique()
...
plot <- ggplot(data = dta1[!cs %in% 0],
               aes(x = BrthYear,
                   ...
                   fill = robson,
                   shape = robson,
                   text = paste(paste0(robsonnam," - ", BrthYear),
                               "<br>Total deliveries:", scales::comma(total, accuracy = 1),
...
reactable(tbl,
  ...
  reactablefmtr::add_source("Note: The Robson criteria for the classification of deliveries into ten mutually exclusive groups by maternal characteristics allows comparison of Caesarean section rates at regional and national levels. Please note that for the purposes of this report: 
...
plot <- ggplot(data = dta1,
               aes(x = BrthYear,
                   ...
                   fill = lab,
                   shape = lab,
                   text = paste(paste0(lab," - ", BrthYear),
...
plot <- ggplot(data = dta1,
               aes(x = BrthYear,
                   ...
                   fill = vbac4,
                   shape = vbac4,
                   text = paste(paste0(vbac4," - ", BrthYear),
...
dta1 <- dta[, .(BrthYear,
  ...
  .[, `:=` (total = .N), by = list(BrthYear, parity)] %>%
  .[, `:=` (rate = count/total)] %>%
  .[,.(BrthYear, rate, total, parity, epi)] %>%
  unique()
...
plot <- ggplot(data = dta1[obs_int %in% "Yes"],
               aes(x = BrthYear,
                   ...
                   text = paste(BrthYear,
                               "<br>Total deliveries", scales::comma(total, accuracy = 1),
                               "<br>Obstetrical intervention (%):" , scales::percent(rate, accuracy = .1)
                               )
                   )
               ) +
