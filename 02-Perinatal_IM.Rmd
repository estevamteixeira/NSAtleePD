...
dta1 <- unique(dta[, .(BrthYear,
  ...
  .[, `:=` (perimort = fifelse(BTOUTCOM %in% c("FTD"), 1,
                               fifelse(BTOUTCOM %in% c("END"), 2, 0)))] %>%
...
tbl <- unique(dta[, .(BrthYear,
  ...
  .[, `:=` (babyout = fifelse(is.na(DIFINOUT) & !is.na(BTOUTCOM),
                              BTOUTCOM,
                              DIFINOUT))] %>%
  .[, `:=` (perimort = fifelse(tolower(babyout) %in% c("ftd"), 1,
                       fifelse(tolower(babyout) %in% c("end"), 2, 
                        fifelse(tolower(babyout) %in% c("pid"), 3, 0))))] %>%
  .[, `:=` (perimortT = .N), by = list(BrthYear, perimort)] %>%
  .[, `:=` (dlv = .N), by = list(BrthYear)] %>%
  .[!perimort %in% 3] %>%
  .[, `:=` (rate = 1000*perimortT/dlv)] %>%
...
dta1 <- unique(dta[, .(BrthYear,
  ...
  .[, `:=` (babyout = fifelse(is.na(DIFINOUT) & !is.na(BTOUTCOM),
                              ...
                              DIFINOUT))] %>%
  #.[!tolower(babyout) %in% c("pid")] %>%
  .[, `:=` (infmort = fifelse(tolower(babyout) %in% c("lvd"), 0,
                      fifelse(tolower(babyout) %in% c("end"), 2, 
                      fifelse(tolower(babyout) %in% c("pid"), 3, 1))))] %>%
