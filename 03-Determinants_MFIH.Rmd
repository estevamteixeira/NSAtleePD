...
plot <- ggplot(data = dta1[!matcat %in% c("20 \u2264 age < 35")],
               aes(x = BrthYear,
                   ...
                   fill = matcat,
                   linetype = parity,
                   text =paste(paste0(matcat," - ",parity),
                               "<br>Year:", BrthYear,
                               "<br>Total deliveries:", scales::comma(total, accuracy = 1),
...
tbl <- as.data.frame(sapply(tbl, sprintf, fmt = c("%.0f", "%1.1f%%", "%1.1f%%")))
# names(tbl) <- coln
tbl[,1] <- c("Total deliveries to primiparous women","age < 20", "age \u2265 35")
...
#names(tbl) <- coln
tbl[,1] <- c("Total deliveries","0","1","\u2265 2")
...
# names(tbl) <- coln
tbl[,1] <- c("Total deliveries","Partnered")
...
tbl[,1] <- c("Total deliveries","Smoking")
...
tbl <- as.data.frame(sapply(tbl, sprintf, fmt = c("%.0f", "%1.1f%%", "%1.1f%%", "%1.1f%%")))
# names(tbl) <- coln
tbl[,1] <- c("Total deliveries to smokers", "Quit", "Reduced", "No change or increased")
...
reactable(tbl,
  ...
  reactablefmtr::add_source("Among women who were known to be smokers at the time of their first prenatal visit, and for whom amount smoked was known at both the first prenatal visit and at delivery.",
                            font_size = 12,
                            #font_weight = "bold"
  )
```
...
#names(tbl) <- coln
tbl[,1] <- c("Total deliveries", "Underweight", "Normal weight", "Overweight", "Obese")
...
tbl <- as.data.frame(sapply(tbl, sprintf, fmt = c("%.0f", "%1.1f%%", "%1.1f%%", "%1.1f%%")))
#names(tbl) <- coln
tbl[,1] <- c("Total deliveries", "Obese I (30 \u2264 BMI < 35)", "Obese II (35 \u2264 BMI < 40)", "Obese III (BMI \u2265 40)")
...
tbl <- as.data.frame(sapply(tbl, sprintf, fmt = c("%.0f", "%1.1f%%", "%1.1f%%", "%1.1f%%")))
#names(tbl) <- coln
tbl[,1] <- c("Total deliveries", "Inadequate", "Adequate", "Excessive")
...
#names(tbl1) <- coln
tbl1[,1] <- c("Total deliveries", c("Inadequate (gain < 12.5 kg)", "Adequate (12.5 kg \u2264 gain \u2264 18 kg)", "Excessive (gain > 18 kg)"))

# add comma separator to total count
tbl1[1,-1] <- as.character(sapply(as.numeric(tbl1[1,-1]), scales::comma_format(accuracy = 1)))

temp <- as.data.frame(t(c("Underweight (BMI < 18.5)", rep(NA, ncol(tbl1)-1))))
names(temp) <- names(tbl1)
...
#names(tbl2) <- coln
tbl2[,1] <- c("Total deliveries", c("Inadequate (gain < 11.5 kg)", "Adequate (11.5 kg \u2264 gain \u2264 16 kg)", "Excessive (gain > 16 kg)"))

# add comma separator to total count
tbl2[1,-1] <- as.character(sapply(as.numeric(tbl2[1,-1]), scales::comma_format(accuracy = 1)))

temp <- as.data.frame(t(c("Normal weight (18.5 \u2264 BMI < 25)", rep(NA, ncol(tbl2)-1))))
names(temp) <- names(tbl2)
...
#names(tbl3) <- coln
tbl3[,1] <- c("Total deliveries", c("Inadequate (gain < 7 kg)", "Adequate (7 kg \u2264 gain \u2264 11.5 kg)", "Excessive (gain > 11.5 kg)"))

# add comma separator to total count
tbl3[1,-1] <- as.character(sapply(as.numeric(tbl3[1,-1]), scales::comma_format(accuracy = 1)))

temp <- as.data.frame(t(c("Overweight (25 \u2264 BMI < 30)", rep(NA, ncol(tbl3)-1))))
names(temp) <- names(tbl3)
...
#names(tbl4) <- coln
tbl4[,1] <- c("Total deliveries", c("Inadequate (gain < 5 kg)", "Adequate (5 kg \u2264 gain \u2264 9 kg)", "Excessive (gain > 9 kg)"))

# add comma separator to total count
tbl4[1,-1] <- as.character(sapply(as.numeric(tbl4[1,-1]), scales::comma_format(accuracy = 1)))

temp <- as.data.frame(t(c("Obese (BMI \u2265 30)", rep(NA, ncol(tbl4)-1))))
...
tbl[,1] <- c("Total deliveries", "Loss > 1 kg", "Within 1 kg", "1 kg < Gain < 5 kg", "Gain \u2265 5 kg")
...
plot <- ggplot(data = dta1[!prehtn %in% "No"],
               aes(x = BrthYear,
                   ...
                   text =paste(prehtn,
                               "<br>Year:", BrthYear,
                               "<br>Total deliveries:", scales::comma(total, accuracy = 1),
                               paste0("<br>",prehtn," (%):") , scales::percent(rate, accuracy = .1)
                               )
                   )
               ) +
...
tbl[,1] <- c("Total deliveries", "None", "1", "2", "\u2265 3")
...
tbl[,1] <- c("Total live births", "Exclusive", "Non-exclusive")
