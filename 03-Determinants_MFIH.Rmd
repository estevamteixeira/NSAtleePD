...
kableExtra::add_footnote(label = c("With known pre-pregnancy weight and height.",
                                   "Note: Body mass index (BMI) is calculated as weight in kilograms divided by the square of height in metres: Underweight, $< 18.5$ $kg/m^2$ ; Normal weight, $18.5$ to $24.9$ $kg/m^2$ ; Overweight, $25.0$ to $29.9$ $kg/m^2$ ; Obese, $\\\\ge30.0$ $kg/m^2$."), escape = FALSE)
```
...
dta1 <- dta[, .(BrthYear,
  ...
  .[, idx_244 := rowSums(.SD, na.rm = TRUE), .SDcols = dput(names(dta)[which(grepl("MO2440.", names(dta)))])] %>%
  .[, idx_245 := rowSums(.SD, na.rm = TRUE), .SDcols = dput(names(dta)[which(grepl("MO2450.", names(dta)))])] %>%
  .[, idx_246 := rowSums(.SD, na.rm = TRUE), .SDcols = dput(names(dta)[which(grepl("MO2460.", names(dta)))])] %>%
  .[, idx_247 := rowSums(.SD, na.rm = TRUE), .SDcols = dput(names(dta)[which(grepl("MO2470.", names(dta)))])] %>%
  .[, idx_248 := rowSums(.SD, na.rm = TRUE), .SDcols = dput(names(dta)[which(grepl("MO2480.", names(dta)))])] %>%
  .[, `:=` (predm_nsapd = fcase(
