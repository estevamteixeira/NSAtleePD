...
plot <- ggplot(data = dta1[!matcat %in% c("20 \u2264 age < 35")],
               aes(x = BrthYear,
                   ...
                   fill = matcat,
                   shape = matcat,
                   text =paste(paste0(matcat, " - ",BrthYear),
                               "<br>Total deliveries:", scales::comma(total, accuracy = 1),
                               paste0("<br>",matcat," (%):") , scales::percent(rate, accuracy = .1)
                               )
                   )
               ) +
...
```{r plot32, fig.cap= paste('Maternal age by parity and year, Nova Scotia,', paste0(params$year1,"-",params$year2))}
...
dta1 <- dta[, .(BrthYear,
  ...
  .[, `:=` (rate = count/total)] %>%
  .[,.(BrthYear, rate, total, matcat, parity)] %>%
  unique()
...
plot <- ggplot(data = dta1[!matcat %in% c("20 \u2264 age < 35")],
               aes(x = BrthYear,
                   ...
                   text =paste(paste0(matcat," - ",parity, " - ", BrthYear),
                               "<br>Total deliveries:", scales::comma(total, accuracy = 1),
                               paste0("<br>",matcat," - ",parity," (%):") , scales::percent(rate, accuracy = .1)
                               )
                   )
               ) +
  geom_point(aes(color = matcat, fill = matcat, shape = matcat)) +
...
plot <- ggplot(data = dta1[!partner %in% "0"],
               aes(x = BrthYear,
                   ...
                   color = partner,
                   fill = partner,
                   text =paste(paste0(partner," - ", BrthYear),
                               "<br>Total births:", scales::comma(total, accuracy = 1),
                               "<br>Partenered (%):" , scales::percent(rate, accuracy = .1)
                               )
                   )
               ) +
...
# names(tbl) <- coln
tbl[,1] <- c("Total deliveries\u1d43","Partnered")
...
reactable(tbl,
  ...
  reactablefmtr::add_source("Note: Partnered denotes women who are 'married', who are in a 'common-law' relationship, or who have a 'partner, not othwerwise specified'. The 'partner' option started to be collected on April 1\u02e2\u1d57, 2020.",
                            font_size = 12,
                            #font_weight = "bold"
  )
```
...
```{r plot35, fig.cap= paste('Maternal smoking during pregnancy by year, Nova Scotia,', paste0(params$year1,"-",params$year2))}
...
plot <- ggplot(data = dta1[!smoker %in% "0"],
               aes(x = BrthYear,
                   ...
                   color = smoker,
                   fill = smoker,
                   text =paste(paste0(smoker," - ", BrthYear),
                               "<br>Total deliveries:", scales::comma(total, accuracy = 1),
                               "<br>Smoking (%):" , scales::percent(rate, accuracy = .1)
                               )
                   )
               ) +
...
plot <- ggplot(data = dta1,
               aes(x = BrthYear,
                   ...
                   shape = change,
                   fill = change,
                   text =paste(paste0(change," - ", BrthYear),
                               "<br>Total deliveries:", scales::comma(total, accuracy = 1),
                               paste0("<br>",change, " (%):") , scales::percent(rate, accuracy = .1)
                               )
                   )
               ) +
...
# kableExtra::kable_styling(font_size = 12, full_width = FALSE,
#                           bootstrap_options = c("hover","condensed", "responsive")) %>%
# kableExtra::add_footnote(label = c("Among women who were known to be smokers at the time of their first prenatal visit, and for whom amount smoked was known at both the first prenatal visit and at delivery."))
...
temp <- as.data.frame(t(c("Pre-pregnancy", rep(NA, ncol(tbl1)-1))))
names(temp) <- names(tbl1)
...
temp <- as.data.frame(t(c("First visit", rep(NA, ncol(tbl2)-1))))
names(temp) <- names(tbl2)
...
temp <- as.data.frame(t(c("Delivery", rep(NA, ncol(tbl3)-1))))
...
```{r plot38, fig.cap= paste('Cannabis use recorded during pregnancy by year, Nova Scotia,', paste0(params$year1,"-",params$year2))}
...
plot <- ggplot(data = dta1[!tolower(marij) %in% "no"],
               aes(x = BrthYear,
                   y = rate,
                   group = marij,
                   color= marij,
                   #shape = marij,
                   fill = marij,
                   text =paste(paste0(marij," - ", BrthYear),
                               "<br>Total deliveries:", scales::comma(total, accuracy = 1),
                               "<br>Reported cannabis (%):" , scales::percent(rate, accuracy = .1)
                               )
                   )
               ) +
...
reactable(tbl,
          ...
          defaultColDef = colDef(align = "right",
                                 ...
                                 maxWidth = 180),
          # rowStyle = function(index){
          #   if (index == nrow(tbl)-2) list(color = "black",
          #                                backgroundColor = "#e1e1e1",
          #                                fontWeight = "bold",
          #                                align = "center",
          #                                borderBottom = "#dddddd")
...
plot <- ggplot(data = dta1[!oat %in% "No"],
               aes(x = BrthYear,
                   ...
                   #shape = oat,
                   fill = oat,
                   text =paste(paste0(oat, " - ",BrthYear),
                               "<br>Total deliveries:", scales::comma(total, accuracy = 1),
                               "<br>Opioid agonist (%):" , scales::percent(rate, accuracy = .01)
                               )
                   )
               ) +
...
plot <- ggplot(data = dta1,
               aes(x = BrthYear,
                   ...
                   shape = bmipp,
                   fill = bmipp,
                   text =paste(paste0(bmipp, " - ",BrthYear),
                               "<br>Total deliveries:", scales::comma(total, accuracy = 1),
                               paste0("<br>",bmipp," (%):") , scales::percent(rate, accuracy = .1)
                               )
                   )
               ) +
...
# Specifying colors by age group
  ...
  scale_y_continuous(#trans = 'log2',
    ...
    labels = scales::percent_format(accuracy = 1),
    limits = c(0, NA)) +
  labs(y = 'Proportion of singleton deliveries', x = '') +
  theme_classic()
...
```{r plot313, fig.cap= paste('Gestational weight gain by pre-pregnancy body mass index and year, Nova Scotia,', paste0(params$year1,"-",params$year2))}
...
temp <- as.data.frame(t(c("Underweight", rep(NA, ncol(tbl1)-1))))
names(temp) <- names(tbl1)
...
temp <- as.data.frame(t(c("Normal weight", rep(NA, ncol(tbl2)-1))))
names(temp) <- names(tbl2)
...
temp <- as.data.frame(t(c("Overweight", rep(NA, ncol(tbl3)-1))))
names(temp) <- names(tbl3)
...
temp <- as.data.frame(t(c("Obese", rep(NA, ncol(tbl4)-1))))
...
reactable(tbl,
          ...
          defaultColDef = colDef(align = "right",
                                 ...
                                 maxWidth = 180),
          rowStyle = function(index){
            if (index %in% (seq(1,nrow(tbl),by=5))) list(color = "black",
                                         backgroundColor = "#e1e1e1",
                                         fontWeight = "bold",
                                         align = "center",
                                         borderBottom = "#dddddd")
...
```{r plot315,echo=FALSE, fig.cap= paste('Interpregnancy weight change by year, Nova Scotia,', paste0(params$year1,"-",params$year2))}
...
plot <- ggplot(data = dta1,
               aes(x = BrthYear,
                   ...
                   shape = ipwc_cat,
                   fill = ipwc_cat,
                   text =paste(paste0(ipwc_cat," - ", BrthYear),
                               "<br>Total deliveries:", scales::comma(total, accuracy = 1),
                               paste0("<br>",ipwc_cat," (%):") , scales::percent(rate, accuracy = .1)
                               )
                   )
               ) +
...
plot <- ggplot(data = dta1[!prexdiab %in% "No"],
               aes(x = BrthYear,
                   ...
                   color= prexdiab,
                   fill = prexdiab,
                   text =paste(paste0(prexdiab," - ",BrthYear),
                               "<br>Total deliveries:", scales::comma(total, accuracy = 1),
                               paste0("<br>",prexdiab," (%):") , scales::percent(rate, accuracy = .01)
                               )
                   )
               ) +
...
plot <- ggplot(data = dta1[!prehtn %in% "No"],
               aes(x = BrthYear,
                   ...
                   color= prehtn,
                   fill = prehtn,
                   text =paste(paste0(prehtn," - ", BrthYear),
                               "<br>Total deliveries:", scales::comma(total, accuracy = 1),
                               paste0("<br>",prehtn," (%):") , scales::percent(rate, accuracy = .01)
                               )
                   )
               ) +
...
plot <- ggplot(data = dta1,
               aes(x = BrthYear,
                   ...
                   fill = pprf,
                   shape = pprf,
                   text =paste(paste0(pprf, " - ",BrthYear),
                               "<br>Total deliveries:", scales::comma(total, accuracy = 1),
                               paste0("<br>",pprf," (%):") , scales::percent(rate, accuracy = .1)
                               )
                   )
               ) +
...
plot <- ggplot(data = dta1[!depanx %in% "No"],
               aes(x = BrthYear,
                   ...
                   fill = depanx,
                   #shape = depanx,
                   text =paste(paste0(depanx," - ",BrthYear),
                               "<br>Total deliveries:", scales::comma(total, accuracy = 1),
                               paste0("<br>",depanx," (%):") , scales::percent(rate, accuracy = .1)
                               )
                   )
               ) +
...
plot <- ggplot(data = dta1,
               aes(x = BrthYear,
                   ...
                   fill = bf,
                   shape = bf,
                   text =paste(paste0(bf," - ", BrthYear),
                               "<br>Total deliveries:", scales::comma(total, accuracy = 1),
                               paste0("<br>",bf," (%):") , scales::percent(rate, accuracy = .1)
                               )
                   )
               ) +
